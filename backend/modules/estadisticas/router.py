...
def alertas_inasistencias(umbral: int = 3, db: Session = Depends(get_db), tenant: TenantContext = Depends(get_current_tenant)):
    return crud.get_alertas_inasistencias(db, tenant.colegio_id, umbral, usuario_id=tenant.usuario_id, rol=tenant.rol)
